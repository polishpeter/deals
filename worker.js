...
:root{--gold:#F5A623;--gold2:#E8961A;--bg:#000000;--bg2:#111111;--bg3:#1a1a1a;--bg4:#222222;--border:#2a2a2a;--border2:#333333;--text:#ffffff;--text2:#ebebeb;--text3:#a0a0a0;--text4:#606060;--green:#30D158;--red:#FF453A;--amber:#FFD60A;--blue:#0A84FF;--purple:#BF5AF2;--radius:14px;--radius-sm:10px;--radius-xs:7px}
...
.login-card{position:relative;background:rgba(17,17,17,0.95);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:48px 44px;width:100%;max-width:400px;box-shadow:0 32px 80px rgba(0,0,0,0.8)}
...
.scard{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:var(--radius);padding:20px;position:relative;overflow:hidden;transition:all 0.2s}
...
.sbox{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:var(--radius-sm);padding:12px;text-align:center}
...
.doc-body{padding:12px 14px;max-height:190px;overflow-y:auto;font-size:11px;color:var(--text3);line-height:1.75;white-space:pre-wrap;font-family:monospace}
...
.agent-log{background:rgba(0,0,0,0.6);border:1px solid rgba(255,255,255,0.06);border-radius:var(--radius);padding:18px;min-height:240px;max-height:420px;overflow-y:auto;font-family:monospace;font-size:11px}
...
function buildHTML(anthropicKey) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
  ...
  <div class="main">
    ...
    <div class="split" id="p-leads">
      ...
      <div style="flex:1;display:flex;flex-direction:column;overflow:hidden">
        <div class="no-sel" id="no-sel"><div style="font-size:36px;opacity:0.2">◎</div><div style="font-size:13px;font-weight:500">Select a lead to view details</div><div style="font-size:11px;color:var(--text4)">or click + Add Lead to get started</div></div>
    ...
    <div class="split" id="p-roland">
      <div class="roland-wrap">
        ...
        <div class="r-sel-bar">
          ...
          <select class="fi" id="r-sel" onchange="setRDeal(this.value)" style="max-width:280px;padding:6px 10px;font-size:11px"><option value="">— No deal selected —</option></select>
    ...
    <div class="page" id="p-agent">
      ...
      <div id="agent-done" style="display:none;margin-top:12px"><div class="green-pill">✅ Run complete — configure Sendr.io in Setup to get real leads flowing.</div></div>
    </div>
    <div class="page" id="p-setup" style="max-width:680px">
      ...
      <div style="font-size:12px;color:var(--text4);margin-bottom:22px">All API keys stored securely in Cloudflare Workers environment — never visible in code.</div>
      <div class="card mb12"><div class="card-title">🔑 Environment Variables</div><div id="s-keys"></div></div>
...
<script>
// ── RUNTIME CONFIG (injected securely by Cloudflare Worker) ───────────────────
...
// ── LOGIN — handled server-side, this just shows the app ──────────────────────
function doLogin(){
  // Credentials verified server-side by Cloudflare Worker
  // This just navigates to the app after server auth
  fetch('/auth',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({u:document.getElementById('lu').value.trim(),p:document.getElementById('lp').value.trim()})
...
function goTab(t){
  ...
  ['pipeline','leads','roland','agent','setup'].forEach(id=>{
    ...
    if(el.classList.contains('page')){el.classList.toggle('on',id===t);el.style.display=id===t?'flex':'none';}
    if(el.classList.contains('split')){el.classList.toggle('on',id===t);el.style.display=id===t?'flex':'none';}
  });
...
function renderPipeline(){
  ...
  document.getElementById('kanban').innerHTML=active.length===0
    ...
    :active.map(st=>{
      ...
      return '<div class="kcol"><div class="kcol-head"><span class="kcol-lbl" style="color:'+c+'">'+esc(st)+'</span><span class="kcol-cnt" style="background:'+c+'18;color:'+c+'">'+leads.filter(l=>l.status===st).length+'</span></div>'+
        leads.filter(l=>l.status===st).map(l=>'<div class="kcard" onclick="openFromPipeline('+l.id+')"><div class="kcard-name">'+esc(l.name)+'</div><div class="kcard-meta">'+esc(l.location||'')+' · '+fmt(l.ebitda)+'</div><div class="kcard-scores"><span style="font-size:9px;color:'+sc(l.dealScore)+';font-weight:700">▲ '+(l.dealScore||'–')+'</span><span style="font-size:9px;color:'+mc(l.motivationScore)+'">🔥 '+(l.motivationScore||'–')+'</span></div></div>').join('')+'</div>';
    }).join('');
}
function openFromPipeline(id){goTab('leads');setTimeout(()=>selectLead(id),80);}

// ── LEADS ─────────────────────────────────────────────────────────────────────
function setFilter(s){filterSt=s;document.querySelectorAll('.fbtn').forEach(b=>b.classList.toggle('on',b.textContent===s));renderList();}
function renderList(){
...
function closeDetail(){sel=null;document.getElementById('detail').style.display='none';document.getElementById('no-sel').style.display='flex';renderList();}

// ── DETAIL ────────────────────────────────────────────────────────────────────
function goDTab(t){dTab=t;document.querySelectorAll('.dtab').forEach(b=>b.classList.toggle('on',b.dataset.d===t));renderDetail();}
...
function renderRSel(){const s=document.getElementById('r-sel');if(!s)return;const prev=s.value;s.innerHTML='<option value="">— No deal selected —</option>'+leads.map(l=>'<option value="'+l.id+'"'+(String(l.id)===prev?' selected':'')+'>'+esc(l.name)+' ('+esc(l.status)+')</option>').join('');}
function setRDeal(id){
  ...
  if(rLead){
    ...
    const g=STAGE_BRIEF[rLead.status]||('You are at '+rLead.status+' stage. Tell me where things stand.');
...
async function sendMsg(){
  ...
  try{
    const res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,system:'${ROLAND_PROMPT.replace(/`/g, '\\`').replace(/\$/g, '\\$')}'+ctx,messages:rMsgs.map(m=>({role:m.role,content:m.content}))})});
    const d=await res.json();
    removeTyping();
    rMsgs.push({role:'assistant',content:d.content?.[0]?.text||'Connection error. Check your Anthropic API key.'});
  }catch(e){removeTyping();rMsgs.push({role:'assistant',content:'Connection error. Make sure your ANTHROPIC_KEY is set in Cloudflare Variables and Secrets.'});}
  renderMsgs();
}
async function genDoc(type){
  ...
  const prompts={
    ...
    dd:'Comprehensive due diligence checklist for acquiring '+l.name+', '+l.industry+' business, '+l.employees+' employees, '+fmt(l.ebitda)+' EBITDA. Roland Frasier framework. Categories: Financial, Legal, Operational, Client/Customer, Team/HR, Technology, Market. Flag high-risk with warning. Be specific.',
    script:'Personalized seller approach script for Peter Kolat contacting '+(l.founder||'the founder')+' at '+l.name+'. Context: '+(l.sourceDetail||'Found through research')+'. Faith: '+l.faithLabel+'. Motivation: '+(l.motivationReasons||[]).join(', ')+'. Roland Frasier style — do NOT reveal acquisition intent upfront. Frame as strategic partnership. Include: opening, rapport builders, discovery questions, handling "are you trying to buy my business?", closing for follow-up. Sound like Peter Kolat talking, not a robot.'
  };
  const titles={loi:'Letter of Intent',dd:'Due Diligence Checklist',script:'Seller Approach Script'};
  try{
    const res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,system:'${ROLAND_PROMPT.replace(/`/g, '\\`').replace(/\$/g, '\\$')}',messages:[{role:'user',content:prompts[type]}]})});
    const d=await res.json();
  ...
  }catch(e){alert('Error. Check your Anthropic API key.');}
...
function renderSetup(){
  ...
  document.getElementById('s-keys').innerHTML=mkRows([['Anthropic API Key','Injected securely by Cloudflare Worker','✅ Secure'],['Dropbox Token','Roland training files — in Cloudflare Variables','✅ Secure'],['Sendr.io API Key','Off-market lead database','✅ Secure'],['Login Credentials','Verified server-side by Worker','✅ Secure'],['Alert Email','realestateblackbook@gmail.com','✅ Set'],['Outreach Email','peter@polishpeter.com','✅ Set'],['Alert Phone','586-321-6507','✅ Set']]);
  document.getElementById('s-status').innerHTML=mkRows([['Cloudflare Worker','Serving dashboard securely','✅ Live'],['GitHub Repo','polishpeter/deals — source backup','✅ Connected'],['Deal Scout CRM','Pipeline + Leads + Kanban + Notes','✅ Live'],['Roland Coach','AI coaching + LOI + DD + Scripts','✅ Live'],['Sourcing Agent','Sendr.io + Google + Social','⏳ Configure Sendr.io'],['Gmail Alerts','Hot lead notifications','⏳ Needs Albato flow'],['SMS Alerts','Text on motivation 80+','⏳ Needs Albato flow'],['GHL Pipeline','Lead Connector sync','⏳ Needs Albato flow']]);
...
export default {
  async fetch(request, env) {
    ...
    // Handle auth endpoint — server-side credential check
    if (url.pathname === '/auth' && request.method === 'POST') {
      try {
        const body = await request.json();
        const validUser = env.DS_USERNAME || '';
        const validPass = env.DS_PASSWORD || '';
        const ok = body.u === validUser && body.p === validPass;
    ...
    // Serve the main app — inject only the Anthropic key for Roland
    const anthropicKey = (env && env.ANTHROPIC_KEY) ? env.ANTHROPIC_KEY : '';
    const html = buildHTML(anthropicKey);
