...
export default {
  async fetch(request, env) {
    ...
    const username = (env && env.DS_USERNAME) ? env.DS_USERNAME : "";
    const password = (env && env.DS_PASSWORD) ? env.DS_PASSWORD : "";
    const anthropicKey = (env && env.ANTHROPIC_KEY) ? env.ANTHROPIC_KEY : "";
    const dropboxToken = (env && env.DROPBOX_TOKEN) ? env.DROPBOX_TOKEN : "";
    const sendrKey = (env && env.SENDR_KEY) ? env.SENDR_KEY : "";

    // Inject secrets into HTML at request time — never stored in code
    let html = HTML;
    html = html.replace(/\{\{DS_USERNAME\}\}/g, username);
    html = html.replace(/\{\{DS_PASSWORD\}\}/g, password);
    html = html.replace(/\{\{ANTHROPIC_KEY\}\}/g, anthropicKey);
    html = html.replace(/\{\{DROPBOX_TOKEN\}\}/g, dropboxToken);
    html = html.replace(/\{\{SENDR_KEY\}\}/g, sendrKey);
